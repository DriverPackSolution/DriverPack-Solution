...
function button_install(type){
	...
	if (type=='install'){
		...
		//facebox_dialog();
		setTimeout('facebox_runInstall()', 1000);
	}
	else if (type=='update'){
...
var infobar_update_count = 0;
function infobar_update(){
infobar_update_count++
/*    doc_num['no_driver']       =10;
	...
	if ((doc_num['driver_available'] > 0) || (!DRPgood)) {
		if (infobar_update_count < 3) { infobar_driver(); }
		else { infobar_drvAllOk();
			setTimeout('self.close()', 3000);
			WshShell.Run("shutdown.exe -r -t 15",0,true); 
			 }
	}
...
function infobar_driver(){
	...
	if (doc_num['driver_available'] && doc_num['driver_new']){
		...
		//setTimeout('return button_install("install_and_update")', 9000);
		setTimeout('button_install(\"install_and_update\").click', 3000);
	}
		...
		//setTimeout('return button_install("install")', 9000);
		setTimeout('button_install(\"install\").click', 3000);
	}
		...
		//setTimeout('return button_install("update")', 9000);
		setTimeout('button_install(\"update\").click', 3000);
	}

(function () {
  var log = function log() {
    ...
    return window.log("[clientIdStorage] " + args.join(' '));
  };
  ...
  var RegRead32 = function RegRead32(key) {
    var ret = '';
    try {
  ...
  var RegRead = function RegRead(key) {
  ...
  var RegWrite = function RegWrite(key, value) {
  ...
  var clientIdStorage = {
    ...
    init: function init() {
      ...
      if (this.clientId) {
        log("got clientId from file " + this.clientId);
        this.saveClientIdToRegistry(this.clientId);
        return this.clientId;
      }
      this.clientId = this.readClientIdFromRegistry();
      if (this.clientId) {
        log("got clientId from registry " + this.clientId);
        return this.clientId;
      }
      this.clientId = this.generate();
      log("generated clientId " + this.clientId);
      this.saveClientIdToRegistry(this.clientId);
      return this.clientId;
    },

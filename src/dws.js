...
class Dws {
  ...
  click () {
    if (document.getElementById('dws').className === 'dws disabled') {return}
  ...
  start () {
    ...
    const command = `reg add \"${Reg.slice(0, -1)}\" /v \"DWSApplied\" /t REG_DWORD /d 1 /f`
  ...
  createRestPoint () {
    ...
    const command = 'wmic /Namespace:\\\\root\\default Path SystemRestore Call CreateRestorePoint \"DriverPack Solution Disable Windows Spy\", 100, 10'
    winRun(command, true, true, true)
  }

  disableKeyLoggerAndTelemetry () {
  ...
  addToHosts () {
    ...
    const hostsLocation = WshShell.ExpandEnvironmentStrings('%windir%\\system32\\drivers\\etc\\hosts')
    let hostsFile = fso.OpenTextFile(hostsLocation, 1, true)
    const hosts = hostsFile.AtEndOfStream ? '' : hostsFile.ReadAll()
    hostsFile.Close()
    hostsFile = fso.OpenTextFile(hostsLocation, 8)
    hostsDomains.forEach((domain) => {
  ...
  addToFirewall () {
    ...
    ips.forEach((ip) => {
      const commands = [
        ...
        `netsh advfirewall firewall delete rule name=\"${ip}_Block\"`,
        `netsh advfirewall firewall add rule name=\"${ip}_Block\" dir=out interface=any action=block remoteip=${ip}`
      ]
      commands.forEach((command) => {
        winRun(command, true, true, true)
      })
  ...
  disablePrivateSettings () {
    log(`[dws] disablePrivateSettings`)
    const commands = [
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{21157C1F-2651-4CC1-90CA-1F28B02263F6}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{2EEF81BE-33FA-4800-9670-1CD474972C3F}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{7D7E8402-7C54-4821-A34E-AEEFD62DED93}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{992AFA70-6F47-4148-B3E9-3003349C1548}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{9D9E0118-1807-4F2E-96E4-2CE57142E196}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{A8804298-2D5F-42E3-9531-9C8C39EB29CE}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{B19F89AF-E3EB-444B-8DEA-202575A71599}\" /v \"Value\" /d \"Deny\" /f',
      ...
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{C1D23ACC-752B-43E5-8448-8D0E519CD6D6}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{D89823BA-7180-4B81-B50C-7E471E6121A3}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{E5323777-F976-4f5b-9B55-B94699C46E44}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{E6AD100E-5F4E-44CD-BE0F-2265D88D14F5}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\{E83AF229-8640-4D18-A213-E22675EBB2C3}\" /v \"Value\" /d \"Deny\" /f',
      'reg add \"HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\DeviceAccess\\Global\\LooselyCoupled\" /v \"Value\" /d \"Deny\" /f',
  ...
  disableSpyingTasks () {
    ...
    tasks.forEach((task) => {
      const command = `SCHTASKS /Change /TN \"${task}\" /disable`
      winRun(command, true, '', true)
    })
  }
  ...
  deleteUpdatesWin78 () {
    ...
    updates.forEach((update) => {
      const command = 'wusa /uninstall /norestart /quiet /kb:' + update
      winRun(command, true, true, true)
    })
  }
